...
namespace Override
{
	public class LinqFilter : Filter
	{
		...
		public Func<Verse.Def, bool> Predicate (XmlAttributeCollection attributes) {
			...
			try {
				var parser = DynamicExpression.ParseLambda<Verse.Def, bool>(expression, itType);

				Log.Message("Override :: Override :: Filter is \"" + itType + " " + parser + "\"");

				return parser.Compile();

			} catch (ParseException e) {
				throw new ArgumentException ("Syntax error", e);
			}
		}
	...
	{
		public static LambdaExpression ParseLambda(Type itType, Type resultType, string expression, Type realType)
		{
			return ParseLambda(Expression.Parameter(itType, ""), resultType, expression, realType);
		}

		public static LambdaExpression ParseLambda(ParameterExpression parameter, Type resultType, string expression, Type realType)
		{
			ExpressionParser parser = new ExpressionParser(parameter, expression, realType);
			return Expression.Lambda(parser.Parse(resultType), parameter);
		}

		public static Expression<Func<T, S>> ParseLambda<T, S>(string expression, Type realType = null)
		{
			return (Expression<Func<T, S>>)ParseLambda(typeof(T), typeof(S), expression, realType);
		}
	...
	internal class ExpressionParser
	{
		...
		public ExpressionParser(ParameterExpression parameter, string expression, Type realType)
		{
		...
		public Expression Parse(Type resultType)
		{
			...
			if (type != null)
				expr = Expression.AndAlso (Expression.TypeIs (it, type), expr);
			if (resultType != null)
		...
		Expression ParseMemberAccess(Type type, Expression instance)
		{
			...
			else if (instance != null) type = instance.Type;
			int errorPos = token.pos;
			...
			if (token.id == TokenId.OpenParen)
			{
				...
				case 1:
					if (type != null)
						instance = Expression.TypeAs (instance, this.type);
